...
Connection.kMaxMessageSize = 10 * 1024 * 1024; // 10MB

Connection.TransactionStatus = {
	IDLE   : 0,
	ONGOING: 1,
	FAILED : 2,
};
...
class Connection {
	constructor(options) {
		...
		this._write(packets.StartupMessage(this));
	}
	...
	_recv(data) {
		 ...
		 */
		if (this._currentMessageLength === -1 && this._aggregator.length >= 5) {
			const buffer = this._aggregator.join();
			const type   = buffer.readUInt8(0, true);
			const size   = buffer.readInt32BE(1, true) + 1; // the [size] field includes itself but not the Byte1 [type] field above
			const parser = parsers.UINT8[type];

			if (!parser || size < 5 || size > Connection.kMaxMessageSize) {
		...
		if (this._currentMessageLength > 0 && this._aggregator.length >= this._currentMessageLength) {
			...
			// initiate/get the reader/parser
			...
			try {
				// skip the message header (Byte1 [type] & Int32 [size])
				reader.advance(5);

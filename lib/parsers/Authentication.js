...
function AuthenticationOkParser(conn, reader) {
	...
	if (conn._pendingQueryPackets.length) {
		debug.enabled && debug('>>>', `AuthenticationOkParser sending ${conn._pendingQueryPackets.length} queued packets`);

		conn._pendingQueryPackets.forEach(conn._write, conn);
		conn._pendingQueryPackets = null;
	}
}
...
function AuthenticationCleartextPasswordParser(conn, reader) {
	...
	conn._write(packets.PasswordMessage(conn.options.password));
}
...
function AuthenticationMD5PasswordParser(conn, reader) {
	...
	conn._write(packets.PasswordMessage(result));
}
...
function AuthenticationParser(conn, reader) {
	...
	debug.enabled && debug('>>>', `AuthenticationParser => ${subParser.name}`);

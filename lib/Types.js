...
function TypeParser$bool$text(buffer) {
	return buffer[0] === 0x74;
}

function TypeParser$bool$binary(buffer) {
	return buffer[0] !== 0x00;
}

function TypeParser$bytea$text(buffer) {
	return new Buffer(buffer.toString('ascii').slice(2), 'hex');
}

function TypeParser$bytea$binary(buffer) {
	return buffer;
}

function TypeParser$text$universal(buffer) {
	return buffer.toString();
}

function TypeParser$number$text(buffer) {
	return Number(buffer.toString()); // TODO: bignum for int8
}

function TypeParser$int2$binary(buffer) {
	return buffer.readInt16BE(0);
}

function TypeParser$int4$binary(buffer) {
	return buffer.readInt32BE(0);
}

function TypeParser$int8$binary(buffer) {
	return buffer.toString(); // TODO: bignum for int8
}

function TypeParser$float4$binary(buffer) {
	return buffer.readFloatBE(0);
}

function TypeParser$float8$binary(buffer) {
...
function TypeParser$timestamp$text(buffer) {
	return new Date(buffer.toString());
}

function TypeParser$timestamptz$text(buffer) {
	/*
	 * postgres formats ISO dates as '2015-09-24 19:03:07.626+02'
	 * ---> v8 interprets '+02' as +2 minutes instead of hours
	 */
	return new Date(buffer.toString() + ':00');
}

function TypeParser$timestamp$binary(buffer) {

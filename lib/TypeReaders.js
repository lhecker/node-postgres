...
function TypeReader$bool$text(reader) {
	return reader.getUInt8() === 0x74;
}

function TypeReader$bool$binary(reader) {
	return reader.getUInt8() !== 0x00;
}

function TypeReader$bytea$text(reader) {
	return new Buffer(reader.toString('ascii').slice(2), 'hex');
}

function TypeReader$bytea$binary(reader) {
	return reader.toBuffer();
}

function TypeReader$text$universal(reader) {
	return reader.toString();
}

function TypeReader$number$text(reader) {
	return Number(reader.toString('ascii')); // TODO: int8 support
}

function TypeReader$int2$binary(reader) {
	return reader.getInt16();
}

function TypeReader$int4$binary(reader) {
	return reader.getInt32();
}

function TypeReader$int8$binary(reader) {
	return reader.toBuffer(); // TODO: implement
}

function TypeReader$float4$binary(reader) {
	return reader.getFloat();
}

function TypeReader$float8$binary(reader) {
	return reader.getDouble();
}

function TypeReader$timestamp$text(reader) {
	return new Date(reader.toString('ascii'));
}

function TypeReader$timestamp$binary(reader) {
	/*
	 * - Since JavaScript does not support 64 Integers we must use it's Doubles.
	 * - We must discard the µsecs since JavaScript's uses msecs.
	 * - We do not use MessageReader's getInt64() method since we can improve the precision thousandfold
	 *   by applying the divisor on the 32 MSB directly instead of doing it later on.
...
function TypeReader$timestamptz$text(reader) {
	/*
...
const _$_default_$_              = [TypeReader$text$universal,   TypeReader$text$universal    ];
const types = [];
